...
import { hostContent } from '@/data/site'
  ...
  if (!episode) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-cream px-6">
        <h1 className="font-hand text-4xl font-semibold text-bronze">Episode not found</h1>
        <Link
          to="/"
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <header className="relative h-[70vh] min-h-[480px] overflow-hidden">
        ...
        <div className="relative flex h-full flex-col justify-end px-6 pb-12 pt-28 md:px-12 lg:px-20">
          <span className="font-mono text-xs uppercase tracking-[0.3em] text-cream/70">
            ...
            {episode.readingTime ?? '42 min listen'}
          </span>
          <EditorialReveal
            as="h1"
            className="mt-3 max-w-4xl font-hand text-4xl font-semibold leading-tight tracking-tight text-cream md:text-6xl lg:text-7xl"
          >
            {episode.title}
          </EditorialReveal>
      ...
      <main className="mx-auto grid max-w-6xl gap-12 px-6 py-16 md:grid-cols-[1fr_280px] md:px-12 lg:gap-20">
        <article>
          <p id="overview" className="text-lg leading-relaxed text-bronze-muted">
            {episode.description}
          </p>

          <blockquote className="my-12 border-l-2 border-bronze/30 py-2 pl-6 font-hand text-2xl font-semibold italic leading-snug text-bronze md:text-3xl">
            {pullQuote}
          </blockquote>
          ...
          <div id="listen">
            <CustomAudioPlayer
              episode={episode}
              chapters={chapters}
              className="sticky top-24 z-10 md:static"
            />
          </div>

          <section id="host" className="mt-16">
          ...
          {episode.authorNotes && (
            <div className="mt-12 border border-bronze/10 bg-cream-dark/50 p-6">
              <p className="font-mono text-[10px] uppercase tracking-widest text-bronze-muted">
                Author&apos;s note
              </p>
          ...
          <div className="mt-12 flex flex-wrap gap-4">
            ...
            <button
              type="button"
              className="border border-bronze/20 px-6 py-2.5 text-xs uppercase tracking-widest text-bronze"
            >
              Download
            </button>
          </div>
            ...
            <div className="mt-8 grid gap-6 sm:grid-cols-3">
              {related.map((ep) => (
                ...
                >
                  ...
                  />
                  <p className="mt-2 font-hand text-lg font-semibold text-bronze group-hover:underline">
                    {ep.title}
                  </p>
                </Link>
              ))}
          ...
          <nav className="sticky top-28 space-y-3 border-l border-bronze/15 pl-4">
            ...
            {[
              ...
              <a
                key={item.href}
                href={item.href}
                className="block text-sm text-bronze-muted hover:text-bronze"
              >
                {item.label}
              </a>
            ))}
